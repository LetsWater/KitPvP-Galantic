...
public class hoodKit_2 {

    public void giveHoodKit_2(UUID uuid){
        Player p = Bukkit.getServer().getPlayer(uuid);

        ItemStack ironSword = new ItemStack(new ItemBuilder(Material.IRON_SWORD, 1).setDisplayName("&bHood").setLore("&8- &fLevel: &e2")
                .setUnbreakable().setItemFlag(ItemFlag.HIDE_UNBREAKABLE).build());

        ItemStack bow = new ItemStack(new ItemBuilder(Material.BOW, 1).setDisplayName("&bHood").setLore("&8- &fLevel: &e2")
        ...
        ItemStack leatherHelmet = new ItemStack(new ItemBuilder(Material.LEATHER_HELMET, 1).setColor(Color.LIME)
                ...
                .setDisplayName("&bHood").setLore("&8- &fLevel: &e2").setUnbreakable().setItemFlag(ItemFlag.HIDE_UNBREAKABLE).build());
